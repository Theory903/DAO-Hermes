...
import { api, hermesPluginAssetUrl } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = hermesPluginAssetUrl(manifest.name, manifest.css);
      ...
      const baseUrl = hermesPluginAssetUrl(manifest.name, manifest.entry);
