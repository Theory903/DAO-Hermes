...
/** VOID chat shell uses 28px touch targets under the company topbar inset. */
export const VOID_TITLEBAR_CONTROL_HEIGHT = 28
export const TITLEBAR_CONTROLS_TOP = (TITLEBAR_HEIGHT - TITLEBAR_CONTROL_HEIGHT) / 2

export function titlebarControlsTop(extraTitlebarInset = 0) {
  const controlHeight = extraTitlebarInset > 0 ? VOID_TITLEBAR_CONTROL_HEIGHT : TITLEBAR_CONTROL_HEIGHT
  return Math.max(0, (TITLEBAR_HEIGHT - controlHeight) / 2) + extraTitlebarInset
}

/** Pin the sessions sidebar toggle on the pane divider when docked; else window edge. */
export function titlebarSidebarToggleLeft(
  controlsLeft: number,
  sidebarTrackPx: number,
  controlSizePx: number,
  sidebarDockedOpen: boolean
) {
  if (!sidebarDockedOpen || sidebarTrackPx <= 0) {
    return controlsLeft
  }

  return Math.max(controlsLeft, sidebarTrackPx - controlSizePx)
}
