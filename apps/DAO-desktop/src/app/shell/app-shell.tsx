...
import { DAO_NAV_OWNS_SIDEBAR } from '@/lib/DAO-shell'
...
import {
  ...
  titlebarControlsTop,
  titlebarSidebarToggleLeft,
  VOID_TITLEBAR_CONTROL_HEIGHT
} from './titlebar'
import { TitlebarControls, type TitlebarTool } from './titlebar-controls'

interface AppShellProps {
  children: ReactNode
  /** Pushes Hermes titlebar controls + drag regions below an outer shell topbar (VOID company nav). */
  extraTitlebarInset?: number
...
export function AppShell({
  ...
  const titlebarHeight = TITLEBAR_HEIGHT + extraTitlebarInset
  const titlebarControlsTopPx = titlebarControlsTop(extraTitlebarInset)
  const titlebarControlSizePx =
    extraTitlebarInset > 0 ? VOID_TITLEBAR_CONTROL_HEIGHT : TITLEBAR_CONTROL_HEIGHT
  ...
  const collapsibleLeftPaneOpen = DAO_NAV_OWNS_SIDEBAR
    ? sidebarOpen
    : panesFlipped
      ? fileBrowserOpen
      : sidebarOpen
  ...
  const sidebarDockedOpen =
    !DAO_NAV_OWNS_SIDEBAR &&
    !isSecondaryWindow() &&
    !narrowViewport &&
    !panesFlipped &&
    sidebarOpen
  const sidebarTrackPx = sidebarDockedOpen ? sidebarWidth : 0
  const sidebarToggleLeftPx = titlebarSidebarToggleLeft(
    titlebarControls.left,
    sidebarTrackPx,
    titlebarControlSizePx,
    sidebarDockedOpen
  )
  const paneHoverRevealLeft = DAO_NAV_OWNS_SIDEBAR ? TITLEBAR_EDGE_INSET : sidebarToggleLeftPx
  ...
  const SYSTEM_TOOL_COUNT = DAO_NAV_OWNS_SIDEBAR ? 0 : 4
  ...
  return (
    <SidebarProvider
      className="h-full min-h-0 flex-1 flex-col bg-background"
      ...
      style={
        {
          ...
          '--titlebar-controls-top': `${titlebarControlsTopPx}px`,
          '--titlebar-sidebar-toggle-left': `${sidebarToggleLeftPx}px`,
          '--titlebar-sidebar-docked': sidebarDockedOpen ? '1' : '0',
          '--hermes-titlebar-band-height': `${TITLEBAR_HEIGHT}px`,
          '--void-topbar-height': extraTitlebarInset > 0 ? `${extraTitlebarInset}px` : '0px',
          '--pane-hover-reveal-left': `${paneHoverRevealLeft}px`,
          '--pane-hover-reveal-width': extraTitlebarInset > 0 ? `${titlebarControlSizePx}px` : 'var(--titlebar-control-size)',
    ...
    >
      ...
      <main className="relative z-3 flex min-h-0 w-full flex-1 flex-col overflow-hidden transition-none">
        <PaneShell className="min-h-0 flex-1">
          ...
          <div
            aria-hidden="true"
            className="pointer-events-none absolute top-0 z-1 h-(--titlebar-height) left-[calc(var(--titlebar-controls-left)+var(--titlebar-control-size)+0.25rem)] right-[calc(var(--titlebar-tools-right)+var(--titlebar-tools-width)+0.75rem)] [-webkit-app-region:drag]"
          />
