...
import { chatMessageArraysEquivalent, reconcileResumeMessages } from '@/lib/resume-messages'
import { normalizePersonalityValue } from '@/lib/chat-runtime'
import {
  isVoiceBucketSession,
  markVoiceSessionLineage,
  VOICE_SESSION_SOURCE_ID,
  withImmutableVoiceSource
...
function patchSessionWorkspace(sessionId: string, cwd: string | undefined) {
  ...
  const patch = (session: SessionInfo) => (session.id === sessionId ? { ...session, cwd } : session)

  setSessions(prev => prev.map(patch))
  setVoiceSessions(prev => prev.map(patch))
}

function upsertSessionRow(session: SessionInfo, storedSessionId: string) {
  const previous = findSessionByStoredId(storedSessionId)
  const row = withImmutableVoiceSource(session, previous)
  const lineage = row._lineage_root_id ?? row.id

  const dedupe = (prev: SessionInfo[]) => [
    row,
  ...
  if (isVoiceBucketSession(row)) {
    setVoiceSessions(dedupe)
    setSessions(prev => prev.filter(existing => !sessionMatchesStoredId(existing, storedSessionId)))

    return
  }

  setSessions(dedupe)
  setVoiceSessions(prev => prev.filter(existing => !sessionMatchesStoredId(existing, storedSessionId)))
}

function upsertResolvedSession(session: SessionInfo, storedSessionId: string) {
  upsertSessionRow(session, storedSessionId)
}

async function resolveStoredSession(storedSessionId: string): Promise<SessionInfo | undefined> {
  const cached = findSessionByStoredId(storedSessionId)
...
export function useSessionActions({
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      if (cachedRuntimeId && cachedState) {
        const stored = findSessionByStoredId(storedSessionId)
      ...
      if (stored && isVoiceBucketSession(stored)) {
        markVoiceSessionLineage(stored)
      }
      ...
      try {
        ...
        const messagesBeforeResume = $messages.get()
        let localSnapshot = messagesBeforeResume
        ...
        const resumePromise = requestGateway<SessionResumeResponse>('session.resume', {
          ...
          ...(storedForProfile && isVoiceBucketSession(storedForProfile)
        ...
        // REST prefetch lacks in-flight reasoning; always reconcile against the
        // transcript captured before resume (and the live cache when present).
        const cachedRuntimeId = runtimeIdByStoredSessionIdRef.current.get(storedSessionId)
        const cachedMessages = cachedRuntimeId
          ? sessionStateByRuntimeIdRef.current.get(cachedRuntimeId)?.messages
          : undefined
        const localBaseline =
          cachedMessages && cachedMessages.length >= messagesBeforeResume.length
            ? cachedMessages
        ...
        const preferredMessages =
          localSnapshot.length > 0
            ? preserveLocalAssistantErrors(
                reconcileResumeMessages(localSnapshot, localBaseline),
                localBaseline
              )
            : (() => {
                const resumedMessages = preserveLocalAssistantErrors(
                  reconcileResumeMessages(toChatMessages(resumed.messages), localBaseline),
                  localBaseline
                )

                return chatMessageArraysEquivalent(currentMessages, resumedMessages)
                  ? currentMessages
                  : resumedMessages
              })()

        const messagesForView = preserveLocalAssistantErrors(preferredMessages, localBaseline)
  ...
  const removeSession = useCallback(
    async (storedSessionId: string) => {
      ...
      const removed = findSessionByStoredId(storedSessionId)
      ...
      setVoiceSessions(prev => prev.filter(session => !sessionMatchesStoredId(session, storedSessionId)))

      if (!removed || !isVoiceBucketSession(removed)) {
      ...
      try {
        ...
        if (removed) {
          upsertSessionRow(removed, storedSessionId)

          if (!isVoiceBucketSession(removed)) {
  ...
  const archiveSession = useCallback(
    async (storedSessionId: string) => {
      ...
      const archived = findSessionByStoredId(storedSessionId)
      const wasSelected = selectedStoredSessionId === storedSessionId
      const previousPinned = $pinnedSessionIds.get()
      // Pins are keyed on the durable lineage-root id; the stored id may be the
      // live tip after compression. Drop both so the pin can't linger.
      ...
      if (!archived || !isVoiceBucketSession(archived)) {
      ...
      try {
        ...
        if (archived) {
          upsertSessionRow(archived, storedSessionId)

          if (!isVoiceBucketSession(archived)) {
            setSessionsTotal(prev => prev + 1)
          }
        }
