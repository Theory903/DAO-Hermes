...
import {
  formatDAOCommandsHelp,
  resolveDAOChatCommand,
} from '@/features/DAO/lib/DAO-chat-commands'
import { DAOSlashNavigate } from '@/features/DAO/lib/DAO-slash-nav'
  ...
  const executeSlashCommand = useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      async function runSlash(commandText: string, sessionHint?: string, recordInput = true): Promise<void> {
        ...
        const DAOResolved = resolveDAOChatCommand(command)

        if (DAOResolved) {
          if (DAOResolved.kind === 'help') {
            const resolved = await withSlashOutput(ctx)
            resolved?.render(formatDAOCommandsHelp())
            return
          }

          if (DAOResolved.kind === 'navigate') {
            const resolved = await withSlashOutput(ctx)
            const ok = DAOSlashNavigate(DAOResolved.screen)
            resolved?.render(
              ok
                ? `Opened ${DAOResolved.screen === 'home' ? 'Home' : DAOResolved.screen}.`
                : 'Could not navigate — open a Space first.',
            )
            return
          }
          ...
          if (busyRef.current) {
            const resolved = await withSlashOutput(ctx)
            resolved?.render('session busy — stop the current turn before running a DAO command')
            return
          }

          if (recordInput) {
            const sid = await ensureSessionId(sessionHint)
            if (sid) {
              appendSessionTextMessage(sid, 'user', command)
            }
          }

          await submitPromptText(DAOResolved.text)
          return
        }
