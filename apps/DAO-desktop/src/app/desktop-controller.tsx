...
import {
  isMessagingSource,
  isVoiceBucketSession,
  LOCAL_SESSION_SOURCE_IDS,
  MESSAGING_SESSION_SOURCE_IDS,
  normalizeSessionSource,
  VOICE_SESSION_SOURCE_ID
...
import { VoidAppShell } from '@/features/DAO/shell/VoidAppShell'
import { DAO_CHAT_FILE_BROWSER, DAO_NAV_OWNS_SIDEBAR, VOID_CHAT_TOPBAR_INSET_PX } from '@/lib/DAO-shell'
...
// The messaging slice is the inverse: drop cron + voice + every local source so
// only external-platform conversations remain, then split per platform in the UI.
const MESSAGING_EXCLUDED_SOURCES = ['cron', VOICE_SESSION_SOURCE_ID, ...LOCAL_SESSION_SOURCE_IDS]
...
function sessionsToKeep(scope?: string): Set<string> {
  ...
  if (active) {
    const session = findSessionByStoredId(active)

    if (session && isVoiceBucketSession(session)) {
      return keep
    }
...
export function DesktopController() {
  ...
  useEffect(() => {
    if (!DAO_CHAT_FILE_BROWSER) {
      setFileBrowserOpen(false)
    }
  }, [])

  useEffect(() => {
    if (DAO_NAV_OWNS_SIDEBAR && $panesFlipped.get()) {
  ...
  // Voice sessions (Home-screen "Jarvis" conversations) as their own slice so
  // they render in a dedicated sidebar section instead of crowding recents.
  const refreshVoiceSessions = useCallback(async () => {
    try {
      const { sessions } = await listAllProfileSessions(VOICE_SECTION_LIMIT, 1, 'exclude', 'recent', 'all', {
        source: VOICE_SESSION_SOURCE_ID
      })

      const strayFromRecents = $sessions.get().filter(isVoiceBucketSession)

      setVoiceSessions(prev => mergeVoiceSessionPage(prev, [...sessions, ...strayFromRecents]))
      if (strayFromRecents.length) {
        setSessions(prev => filterVoiceFromRecents(prev))
      }
    } catch {
      // Non-fatal: the voice section just stays empty/stale.
    }
  }, [])
  ...
  const refreshSessions = useCallback(async () => {
    ...
    try {
      ...
      if (refreshSessionsRequestRef.current === requestId) {
        setSessions(prev =>
          filterVoiceFromRecents(mergeSessionPage(filterVoiceFromRecents(prev), result.sessions, sessionsToKeep()))
        )
    ...
    void refreshVoiceSessions()
  }, [profileScope, refreshCronSessions, refreshCronJobs, refreshMessagingSessions, refreshVoiceSessions])
  ...
  const loadMoreSessionsForProfile = useCallback(async (profile: string) => {
    ...
    setSessions(prev => [
      ...
      ...filterVoiceFromRecents(mergeSessionPage(filterVoiceFromRecents(prev.filter(inKey)), result.sessions, keep))
    ])
  ...
  const toggleSelectedPin = useCallback(() => {
    ...
    const session = findSessionByStoredId(sessionId)
      ...
      for (let index = 0; index < Math.max(1, attempts); index += 1) {
        try {
          ...
          updateSessionState(
            runtimeSessionId,
            state => {
              const messages = preserveLocalAssistantErrors(
                reconcileResumeMessages(toChatMessages(latest.messages), state.messages),
                state.messages
              )
          ...
          const messages = sessionStateByRuntimeIdRef.current.get(runtimeSessionId)?.messages ?? []
  ...
  useEffect(() => onGatewayEvent(handleDesktopGatewayEvent), [handleDesktopGatewayEvent])
  ...
  // DAO keeps sessions on the left — no titlebar flip/swap chrome.
  const sidebarSide = DAO_NAV_OWNS_SIDEBAR ? 'left' : panesFlipped ? 'right' : 'left'
  const railSide = DAO_NAV_OWNS_SIDEBAR ? 'right' : panesFlipped ? 'left' : 'right'
  ...
  return (
    <VoidAppShell>
    <AppShell
      extraTitlebarInset={VOID_CHAT_TOPBAR_INSET_PX}
    ...
    >
      ...
      */}
      {panesFlipped ? (DAO_CHAT_FILE_BROWSER ? fileBrowserPane : terminalPane) : terminalPane}
      {previewPane}
      {panesFlipped ? terminalPane : DAO_CHAT_FILE_BROWSER ? fileBrowserPane : null}
    </AppShell>
    </VoidAppShell>
  )
}
