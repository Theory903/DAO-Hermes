...
export function useSlashCompletions(options: {
  ...
  const fetcher = useCallback(
    async (query: string): Promise<CompletionPayload> => {
      ...
      const DAOPrefix = /^\/([\w-]*)$/i.exec(text)
      if (DAOPrefix) {
        const DAOItems = filterDAOCommands(text).map(entry => ({
          text: entry.name,
          display: entry.name,
        ...
        if (DAOItems.length) {
          if (!query) {
            try {
              ...
              const catalogItems = sections.flatMap(section =>
              ...
              return { items: [...DAOItems, ...catalogItems], query }
            } catch {
              return { items: DAOItems, query }
            }
          }
