...
import { DAOSpaceSwitcher } from '@/features/DAO/shell/DAOSpaceSwitcher'
...
export function ChatSidebar({
  ...
  const voiceOpen = useStore($sidebarVoiceOpen)
  const selectedSessionId = useStore($selectedStoredSessionId)
  const sessions = useStore($sessions)
  const cronSessions = useStore($cronSessions)
  const voiceSessions = useStore($voiceSessions)
  ...
  const voiceSessionIdSet = useMemo(() => new Set(voiceSessions.map(s => s.id)), [voiceSessions])

  const visibleSessions = useMemo(() => {
    const scoped = showAllProfiles ? sessions : sessions.filter(s => normalizeProfileKey(s.profile) === profileScope)

    // Voice lives in its own section — never under Recents / "No workspace".
    return filterVoiceFromRecents(scoped).filter(s => !voiceSessionIdSet.has(s.id))
  }, [sessions, showAllProfiles, profileScope, voiceSessionIdSet])

  const sortedVoiceSessions = useMemo(
    () => [...voiceSessions].sort((a, b) => sessionTime(b) - sessionTime(a)),
    [voiceSessions]
  )

  // Newest activity first (last message / resume), not session creation time.
  ...
  const sessionByAnyId = useMemo(() => {
    ...
    // Cron + voice sessions are listed separately but can still be pinned, so
    // index them too — otherwise a pinned cron/voice session can't resolve into
    // the Pinned section. Recents take precedence on id collisions (set last).
    for (const s of [...cronSessions, ...voiceSessions, ...visibleSessions]) {
      map.set(s.id, s)
    ...
    return map
  }, [visibleSessions, cronSessions, voiceSessions])
  ...
  const searchResults = useMemo(() => {
    ...
    for (const s of sortedVoiceSessions) {
      if (sessionMatchesSearch(s, trimmedQuery)) {
        out.set(s.id, s)
      }
    }
    ...
    return [...out.values()].sort((a, b) => sessionTime(b) - sessionTime(a))
  }, [trimmedQuery, sortedSessions, sortedVoiceSessions, serverMatches, sessionByAnyId])
  ...
  const agentSessions = useMemo(
    () => [...unpinnedAgentSessions].sort((a, b) => sessionTime(b) - sessionTime(a)),
  ...
  return (
    <Sidebar
      className={cn(
        // Fill the Pane / hover-reveal track — w-(--sidebar-width) is 0px when the
        // pane track collapses, which hid the whole session list inside overlays.
        'relative h-full w-full min-w-0 overflow-hidden border-t-0 border-b-0 text-foreground transition-none',
      ...
      collapsible="none"
    >
      <SidebarContent className="DAO-chat-sidebar-content gap-0 overflow-hidden bg-transparent px-3">
        <SidebarGroup className="DAO-sidebar-header-group shrink-0 p-0 pb-2 pt-2">
          <SidebarGroupContent className="DAO-sidebar-actions">
                  ...
                  <SidebarMenuItem key={item.id}>
                    <SidebarMenuButton
                      ...
                      className={cn(
                        ...
                        // thread.tsx.
                        'flex h-8 w-full justify-start gap-2.5 rounded-lg border border-transparent px-2.5 text-left text-[0.8125rem] font-medium text-(--ui-text-secondary) transition-colors duration-100 ease-out [-webkit-app-region:no-drag] hover:bg-(--ui-control-hover-background) hover:text-foreground hover:transition-none',
                        active &&
        ...
        {contentVisible && showSessionSections && (
          <div className="DAO-sidebar-search shrink-0 px-0 pb-2.5 pt-2">
            <SearchField
        ...
        {contentVisible && showSessionSections && (
          <div className={cn('flex min-h-0 flex-1 flex-col gap-1 pb-2', SCROLL_Y)}>
            {trimmedQuery && (
            ...
            {!trimmedQuery && sortedVoiceSessions.length > 0 && (
              <SidebarSessionsSection
                activeSessionId={activeSidebarSessionId}
                contentClassName={cn('flex max-h-56 flex-col gap-px pb-1.75', GROUP_BODY)}
                emptyState={null}
                label="Voice"
                labelMeta={countLabel(sortedVoiceSessions.length, sortedVoiceSessions.length)}
                onArchiveSession={onArchiveSession}
                onDeleteSession={onDeleteSession}
                onResumeSession={onResumeSession}
                onToggle={() => setSidebarVoiceOpen(!voiceOpen)}
                onTogglePin={pinSession}
                open={voiceOpen}
                pinned={false}
                rootClassName="shrink-0 p-0"
                sessions={sortedVoiceSessions}
                workingSessionIdSet={workingSessionIdSet}
              />
            )}
        ...
        {contentVisible && (
          <div className="DAO-sidebar-footer shrink-0 px-0.5 pb-2 pt-1">
            <ProfileRail />
          </div>
        )}
      </SidebarContent>
...
function SidebarSectionHeader({ label, open, onToggle, action, meta, icon }: SidebarSectionHeaderProps) {
  return (
    <div className="group/section flex shrink-0 items-center justify-between px-0.5 pb-1.5 pt-2">
      <button
