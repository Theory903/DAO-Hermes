...
const sessionRecency = (session: SessionInfo) => session.last_active || session.started_at || 0

/** Hermes repo paths read as DAO in the sidebar (product branding). */
export function brandWorkspaceLabel(label: string): string {
  if (!label) return label

  const worktree = label.match(/^hermes-agent-wt-(.+)$/i)

  if (worktree) return `DAO · ${worktree[1]}`

  if (/^hermes-agent$/i.test(label)) return 'DAO'

  return label.replace(/\bhermes-agent\b/gi, 'DAO')
}
...
export function workspaceGroupsFor(
  ...
  for (const group of result) {
    group.label = brandWorkspaceLabel(group.label)
  }
 ...
 * too, while rows inside a worktree sort by most recent activity.
 */
export function workspaceTreeFor(
  ...
  for (const parent of result) {
    ...
    parent.label = brandWorkspaceLabel(parent.label)
