...
import { UtilChipSwitch, UtilChipSwitchItem } from '../util-page-nav'
...
export function ArtifactsView({ setStatusbarItemGroup: _setStatusbarItemGroup, ...props }: ArtifactsViewProps) {
  ...
  return (
    <PageSearchShell
      {...props}
      description={a.pageDesc}
      title={a.pageTitle}
      onSearchChange={setQuery}
      searchHidden={artifacts !== null && counts.all === 0}
      ...
      searchValue={query}
      tabs={
        <UtilChipSwitch aria-label={a.navSections}>
          <UtilChipSwitchItem active={kindFilter === 'all'} count={counts.all} onClick={() => setKindFilter('all')}>
            {a.tabAll}
          </UtilChipSwitchItem>
          <UtilChipSwitchItem active={kindFilter === 'image'} count={counts.image} onClick={() => setKindFilter('image')}>
            {a.tabImages}
          </UtilChipSwitchItem>
          <UtilChipSwitchItem active={kindFilter === 'file'} count={counts.file} onClick={() => setKindFilter('file')}>
            {a.tabFiles}
          </UtilChipSwitchItem>
          <UtilChipSwitchItem active={kindFilter === 'link'} count={counts.link} onClick={() => setKindFilter('link')}>
            {a.tabLinks}
          </UtilChipSwitchItem>
        </UtilChipSwitch>
      }
    >
      {!artifacts ? (
        ...
        <div className="DAO-util-empty h-full">
          <div>
            <div className="DAO-util-empty-title">{a.noArtifactsTitle}</div>
            <div className="DAO-util-empty-desc">{a.noArtifactsDesc}</div>
          </div>
        </div>
      ) : (
        <div className="DAO-util-page-scroll h-full overflow-y-auto">
          <div className={cn('DAO-util-page-content DAO-util-page-content--wide pb-4', PAGE_INSET_X)}>
            {visibleImageArtifacts.length > 0 && (
              <section className="flex flex-col">
                <div className={cn('DAO-artifacts-section-head sticky top-0 z-10 bg-(--ui-chat-surface-background)', PAGE_INSET_NEG_X, PAGE_INSET_X)}>
                  <ArtifactsPagination
                ...
                <div className="DAO-artifacts-grid">
            ...
            {visibleFileArtifacts.length > 0 && (
              <section className="flex flex-col">
                ...
                <div className="DAO-artifact-table-shell">
...
function ArtifactsPagination({ className, itemLabel, onPageChange, page, pageSize, total }: ArtifactsPaginationProps) {
  ...
  return (
    <div className={cn('DAO-artifacts-pagination', className)}>
...
function ArtifactImageCard({ artifact, failedImage, onImageError, onOpenChat }: ArtifactImageCardProps) {
  return (
    <article
      className="DAO-artifact-card"
      onClick={() => onOpenChat(artifact.sessionId)}
      onKeyDown={event => {
        if (event.key === 'Enter' || event.key === ' ') {
          event.preventDefault()
          onOpenChat(artifact.sessionId)
        }
      }}
      role="button"
      tabIndex={0}
    >
      <div className={cn('DAO-artifact-card-media', failedImage && 'cursor-default')}>
        {!failedImage && (
          <ZoomableImage
            alt={artifact.label}
      ...
      <div className="DAO-artifact-card-body">
        <div className="DAO-artifact-card-title">{artifact.label}</div>
        <div className="DAO-artifact-card-meta">
...
function ArtifactTable({
  ...
  return (
    <table className="DAO-artifact-table">
      <thead>
        <tr>
          {ARTIFACT_COLUMNS.map(col => (
            <th className={cn(col.width(filter))} key={col.id}>
