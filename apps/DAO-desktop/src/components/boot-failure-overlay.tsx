...
import { DAOSidecarBaseUrl, isDesktopBridgeAvailable, probeDAOSidecarHealth } from '@/lib/DAO-sidecar-connection'
...
const IPC_UNAVAILABLE = 'Desktop IPC bridge is unavailable.'

function isDAOSidecarBootFailure(error: string | null): boolean {
  if (!error) return false
  if (error === IPC_UNAVAILABLE) return true
  return /DAO api|sidecar|ws\/ticket|9119|fetch failed|network/i.test(error)
}
...
export function BootFailureOverlay() {
  ...
  const DAOSidecarFailure = isDAOSidecarBootFailure(boot.error)
  const sidecarBase = DAOSidecarBaseUrl()
  ...
  useEffect(() => {
    if (!visible || !DAOSidecarFailure) {
      setSidecarHealthy(null)
      return
    }

    let cancelled = false
    void probeDAOSidecarHealth().then(ok => {
      if (!cancelled) setSidecarHealthy(ok)
    })

    return () => {
      cancelled = true
    }
  }, [visible, DAOSidecarFailure, boot.error])
  ...
  const showDAORecovery = DAOSidecarFailure && !remoteReauth
  const title = remoteReauth ? copy.remoteTitle : showDAORecovery ? copy.DAOTitle : copy.title
      ...
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-nous) bg-(--ui-chat-bubble-background) shadow-nous">
        ...
        <div className="grid gap-4 p-5">
          <div className="rounded-2xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-xs text-destructive">
            {boot.error}
            {showDAORecovery && sidecarHealthy === false ? (
              <p className="mt-2 text-(--ui-text-tertiary)">{copy.DAOSidecarHint(sidecarBase)}</p>
            ) : null}
            {showDAORecovery && sidecarHealthy === true ? (
              <p className="mt-2 text-(--ui-text-tertiary)">
                Sidecar health check passed at {sidecarBase}. Retry to reconnect the chat gateway.
              </p>
            ) : null}
          </div>
          ...
          <div className="grid gap-2">
            <div className="flex flex-wrap gap-2">
              {remoteReauth ? (
                ...
                <Button disabled={Boolean(busy)} onClick={() => void retry()}>
                  ...
                  {showDAORecovery ? copy.DAORetrySidecar : copy.retry}
                </Button>
              )}
              {!remoteReauth && !showDAORecovery ? (
            ...
            <p className="text-xs text-muted-foreground">
              {remoteReauth
                ...
                : showDAORecovery
                  ? copy.DAOSidecarHint(sidecarBase)
                  : copy.repairHint}
            </p>
          </div>
