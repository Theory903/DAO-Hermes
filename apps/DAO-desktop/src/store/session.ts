...
/** Look up a stored session across every sidebar bucket (voice, cron, messaging, recents). */
export function findSessionByStoredId(storedSessionId: string): SessionInfo | undefined {
  for (const list of [$voiceSessions.get(), $cronSessions.get(), $messagingSessions.get(), $sessions.get()]) {
    const found = list.find(session => sessionMatchesStoredId(session, storedSessionId))
...
/** Voice sessions live in {@link $voiceSessions} — never in local recents. */
export function filterVoiceFromRecents(sessions: SessionInfo[]): SessionInfo[] {
  return sessions.filter(session => !isVoiceBucketSession(session))
}

/** Merge a voice slice from the server, preserving lineage-tagged rows the API omitted. */
export function mergeVoiceSessionPage(previous: SessionInfo[], incoming: SessionInfo[]): SessionInfo[] {
  const normalized = incoming.map(row => withImmutableVoiceSource(row))

  const incomingLineages = new Set(
    normalized.flatMap(session => [session.id, session._lineage_root_id].filter((id): id is string => Boolean(id)))
  )

  const survivors = previous.filter(session => {
    if (!isVoiceBucketSession(session)) {
      return false
    }

    const lineage = session._lineage_root_id ?? session.id

    return !incomingLineages.has(session.id) && !incomingLineages.has(lineage)
  })

  return [...survivors.map(row => withImmutableVoiceSource(row)), ...normalized].sort(
...
export function mergeSessionPage(
  ...
  if (keep.size === 0) {
    return incoming.map(session => withImmutableVoiceSource(session))
  }

  const previousByLineage = new Map<string, SessionInfo>()

  for (const session of previous) {
    previousByLineage.set(session.id, session)

    if (session._lineage_root_id) {
      previousByLineage.set(session._lineage_root_id, session)
    }
  }

  const normalizeIncoming = (session: SessionInfo) =>
    withImmutableVoiceSource(session, previousByLineage.get(session.id) ?? previousByLineage.get(session._lineage_root_id ?? ''))
  ...
  return survivors.length ? [...survivors, ...incoming.map(normalizeIncoming)] : incoming.map(normalizeIncoming)
}
...
// Voice sessions (source === 'voice') are Home-screen "Jarvis" conversations.
// Fetched as their own slice so they render in a dedicated sidebar section and
// never interleave with typed chats in recents.
export const $voiceSessions = atom<SessionInfo[]>([])
export const VOICE_SECTION_LIMIT = 50
...
export const $lastVisibleMessageIsUser = computed($messages, messages => lastVisibleMessageIsUser(messages ?? []))
...
export const setVoiceSessions = (next: Updater<SessionInfo[]>) => updateAtom($voiceSessions, next)
