...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active DAO profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show DAO agent version (Hermes runtime)', surface: exec() },
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  if (isDAOSlashCommandName(normalized)) {
    return true
  }
...
export function isDesktopSlashSuggestion(command: string): boolean {
  ...
  if (isDAOSlashCommandName(normalized)) {
    return !DAO_CHAT_COMMANDS.some(
      cmd => (cmd.aliases ?? []).includes(normalized),
    )
  }
...
export function desktopSlashDescription(command: string, fallback = ''): string {
  const normalized = normalizeCommand(command)
  const DAO = DAO_CHAT_COMMANDS.find(
    cmd =>
      cmd.name === normalized ||
