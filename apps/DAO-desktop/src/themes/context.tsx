...
import { BUILTIN_THEME_LIST, BUILTIN_THEMES, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, voidTheme } from './presets'
...
const RETIRED_SKINS = new Set(['nous-light', 'default', 'gold', 'nous'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? voidTheme
...
/** VOID uses pure black seeds — no neutral gray bleed. */
const voidMixes = (): Record<string, string> => ({
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...voidTheme.typography, ...theme.typography }
  ...
  root.classList.toggle('DAO-void', skinName === 'void')
  ...
  const palette: Record<string, string> = {
    ...
    '--noise-opacity-mul': skinName === 'void' ? '0' : isDark ? 'calc(0.04 / 0.21)' : 'calc(0.34 / 0.21)'
  }

  for (const [k, v] of Object.entries({
    ...seeds,
    ...(skinName === 'void' ? voidMixes() : mixesFor(isDark)),
    ...palette
  })) {
...
export function useSyncThemeFromBackend(backendThemeName: string | undefined, setTheme: (name: string) => void) {
  useEffect(() => {
    ...
    const mapped = backendThemeName === 'nous' ? 'void' : backendThemeName

    if (BUILTIN_THEMES[mapped]) {
      setTheme(mapped)
    }
