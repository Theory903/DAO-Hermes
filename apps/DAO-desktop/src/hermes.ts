...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'DAO agent connection closed',
      connectErrorMessage: 'Could not connect to DAO agent',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'DAO agent is not connected',
...
export async function searchSessions(query: string): Promise<SessionSearchResponse> {
  const result = await window.hermesDesktop.api<SessionSearchResponse>({
...
export async function getSkills(): Promise<SkillInfo[]> {
  const result = await window.hermesDesktop.api<SkillInfo[]>({
    ...profileScoped(),
    path: '/api/skills'
  })
...
export async function getToolsets(): Promise<ToolsetInfo[]> {
  const result = await window.hermesDesktop.api<ToolsetInfo[]>({
    ...profileScoped(),
...
export async function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  const result = await window.hermesDesktop.api<MessagingPlatformsResponse>({
    path: '/api/messaging/platforms'
  })

  return { ...result, platforms: result?.platforms ?? [] }
}
...
export async function getCronJobs(): Promise<CronJob[]> {
  const result = await window.hermesDesktop.api<CronJob[]>({
    path: '/api/cron/jobs'
  })
...
export async function getProfiles(): Promise<ProfilesResponse> {
  const result = await window.hermesDesktop.api<ProfileInfo[] | ProfilesResponse>({
    path: '/api/profiles'
  })

  return { profiles: Array.isArray(result) ? result : (result?.profiles ?? []) }
}
...
// Audio round-trips hit speech providers (Whisper/TTS) that can be slow on the
// first call (model load) or when proxied to a remote provider, so they get a
// generous timeout instead of the default 15s REST budget.
