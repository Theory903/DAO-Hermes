...
const { registerDAOAuthIpc } = require('./DAO-auth.cjs')
...
// non-macOS platforms.
const NATIVE_OVERLAY_BUTTON_WIDTH = 152
...
function getTitleBarOverlayOptions() {
  ...
  return {
    color: useDarkColors ? '#0a0a0a' : '#f3f3f3',
    height: TITLEBAR_HEIGHT,
    symbolColor: useDarkColors ? '#e8e8e8' : '#1a1a1a',
  }
}
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(timeoutMs, () => {
      const error = new Error(`Timed out connecting to Hermes backend after ${timeoutMs}ms`)
      markBackendUnreachable(error)
      req.destroy(error)
    })
...
ipcMain.handle('hermes:window-state', () => getWindowState())
