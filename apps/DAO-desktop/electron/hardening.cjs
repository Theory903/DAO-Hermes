...
const BACKEND_CIRCUIT_COOLDOWN_MS = 5_000
const DATA_URL_READ_MAX_BYTES = 16 * 1024 * 1024
const TEXT_PREVIEW_SOURCE_MAX_BYTES = 64 * 1024 * 1024

let backendCircuitOpenUntil = 0
let backendCircuitMessage = ''
let onBackendUnreachable = null

function setBackendUnreachableHook(fn) {
  onBackendUnreachable = typeof fn === 'function' ? fn : null
}

function isBackendTransportError(error) {
  const msg = String(error?.message || error || '')
  return /timed out connecting to hermes backend|econnrefused|econnreset|socket hang up|ehostunreach|enotfound/i.test(
    msg
  )
}

function assertBackendReachable() {
  if (Date.now() >= backendCircuitOpenUntil) {
    return
  }

  const error = new Error(
    backendCircuitMessage ||
      'Hermes backend is unreachable. Run ./scripts/dev-desktop.sh to start the API sidecar.'
  )
  error.code = 'BACKEND_UNREACHABLE'
  throw error
}

function markBackendUnreachable(error) {
  ...
  backendCircuitMessage =
    ...
    'Hermes backend is unreachable. Run ./scripts/dev-desktop.sh to start the API sidecar.'
  backendCircuitOpenUntil = Date.now() + BACKEND_CIRCUIT_COOLDOWN_MS

  try {
    onBackendUnreachable?.()
  } catch {
    // Hook failures must not mask the original transport error.
  }
}
...
function resetBackendCircuitForTests() {
  backendCircuitOpenUntil = 0
  backendCircuitMessage = ''
  onBackendUnreachable = null
}
...
module.exports = {
  ...
  resetBackendCircuitForTests,
  resolveDirectoryForIpc,
  resolveReadableFileForIpc,
  resolveRequestedPathForIpc,
  resolveTimeoutMs,
  sensitiveFileBlockReason,
  setBackendUnreachableHook
}
