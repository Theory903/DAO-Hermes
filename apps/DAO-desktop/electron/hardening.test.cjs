...
test('backend circuit breaker fast-fails after transport errors', () => {
  resetBackendCircuitForTests()

  assert.doesNotThrow(() => assertBackendReachable())

  markBackendUnreachable(new Error('Timed out connecting to Hermes backend after 15000ms'))
  assert.throws(() => assertBackendReachable(), error => {
...
test('backend circuit breaker invokes unreachable hook once per trip', () => {
  resetBackendCircuitForTests()
  let calls = 0
  setBackendUnreachableHook(() => {
    calls += 1
  ...
  markBackendUnreachable(new Error('connect ECONNREFUSED 127.0.0.1:9119'))
  markBackendUnreachable(new Error('connect ECONNREFUSED 127.0.0.1:9119'))
  assert.equal(calls, 2)
  assert.equal(isBackendTransportError(new Error('socket hang up')), true)
  assert.equal(isBackendTransportError(new Error('404: not found')), false)
})
