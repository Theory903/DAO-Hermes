...
export default defineConfig({
  ...
  server: {
    ...
    strictPort: true,
    watch: {
      // tsc / IDE incremental builds touch this at the package root and otherwise
      // trigger a full-reload storm across the whole renderer graph.
      ignored: ['**/tsconfig.tsbuildinfo']
    },
    proxy: {
      // Same-origin REST in dev — renderer on :5174, sidecar on :9119.
      '/api/v1': {
        target: process.env.VITE_DAO_API_PROXY_TARGET ?? 'http://127.0.0.1:9119',
        changeOrigin: true
      }
    }
  },
