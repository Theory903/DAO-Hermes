 ...
 *
 * Stamps the DAO icon + identity onto the packed Windows DAO.exe via
 * resedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'DAO'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); DAO.exe keeps the stock Electron icon`)
  }
}
